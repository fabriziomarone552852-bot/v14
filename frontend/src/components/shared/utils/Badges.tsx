...
interface BadgeProps {
  ...
  priority?: 'Alta' | 'Media' | 'Bassa' | string; // Alias per priorityLevel
...
export const Badge: React.FC<BadgeProps> = ({ 
  ...
  const resolvedPriority = priority || priorityLevel;
  ...
  if (variant === 'priority' && resolvedPriority) {
    // 1. Gestione Priorità
    ...
    baseClasses += (priorityStyles[resolvedPriority] || 'bg-gray-100 text-gray-700');
