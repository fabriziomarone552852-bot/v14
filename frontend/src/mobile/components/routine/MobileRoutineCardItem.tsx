// src/mobile/components/routine/MobileRoutineCardItem.tsx
...
interface MobileRoutineCardItemProps {
  routine: RoutineItem;
  onUpdateRoutine: (id: number, delta: number) => void;
  onSelectRoutine: (routine: RoutineItem) => void;
}

export const MobileRoutineCardItem: React.FC<MobileRoutineCardItemProps> = ({
  routine,
  onUpdateRoutine,
  onSelectRoutine,
}) => {
      ...
      >
        {isSingle ? (
          <button
            ...
            aria-label={isCompleted ? 'Annulla completamento' : 'Segna come completata'}
          >
          ...
          <div className="flex items-center bg-black/45 backdrop-blur-xs rounded-xl border border-white/20 overflow-hidden shadow-xs">
            <button
              ...
              aria-label="Diminuisci conteggio"
            >
              -
            </button>
...
export default MobileRoutineCardItem;
